...
private[upgrade] object DeploymentPlanReverter {
  ...
  /**
    ...
    */
  def revert(original: Group, target: Group): Group => Group = {
    ...
    (revertGroupChanges(groupChanges) _).andThen(revertAppChanges(appChanges))
  }
  ...
  /**
    ...
    */
  private[this] def revertGroupChanges(groupChanges: Seq[(Option[Group], Option[Group])])(group: Group): Group = {
    ...
    def revertGroupAddition(result: Group, newGroup: Group): Group = {
      ...
      result.group(newGroup.id) match {
        case Some(unchanged) if isGroupUnchanged(unchanged) =>
          ...
          result.remove(unchanged.id)
        case _ => // group still contains apps/groups or the dependencies have changed
          if (log.isDebugEnabled)
      ...
      case (result, groupUpdate) =>
        groupUpdate match {
          case (Some(oldGroup), None) =>
            result.update(oldGroup.id, revertGroupRemoval(oldGroup), Timestamp.now())

          case (Some(oldGroup), Some(newGroup)) =>
            result.update(oldGroup.id, revertDependencyChanges(oldGroup, newGroup), Timestamp.now())
  ...
  /**
    ...
    */
  private[this] def revertAppChanges(changes: Seq[(Option[AppDefinition], Option[AppDefinition])])(g: Group): Group = {
    changes.foldLeft(g) {
      case (result, appUpdate) =>
        appUpdate match {
          case (Some(oldApp), _) => //removal or change
            log.debug("revert to old app definition {}", oldApp.id)
            result.updateApp(oldApp.id, _ => oldApp, Timestamp.now())
          case (None, Some(newApp)) =>
            log.debug("remove app definition {}", newApp.id)
            result.removeApplication(newApp.id)
          case (None, None) =>
